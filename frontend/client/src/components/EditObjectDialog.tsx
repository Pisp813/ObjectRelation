...
import { insertObjectSchema, ObjectType } from '@shared/schema';
...
export default function EditObjectDialog({ open, onOpenChange, object }: EditObjectDialogProps) {
  ...
  useEffect(() => {
    if (object) {
      setFormData({
        ...
        type: object.type as 'Item' | 'Document',
      });

      // Convert attributes object to array
      const attributesArray = object.attributes && typeof object.attributes === 'object'
  ...
  const updateObjectMutation = useMutation({
    mutationFn: async (data: any) => {
      const validatedData = insertObjectSchema.partial().parse(data);
      return apiRequest('PUT', `/api/objects/${object.id}`, validatedData);
    },
    ...
    onError: (error) => {
      toast({
        title: "Error",
        description: "Failed to update object. Please check your input and try again.",
        variant: "destructive"
      });
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateObjectMutation.mutate(submitData);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="edit-object-dialog">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-end space-x-3 pt-4">
            ...
            >
              {updateObjectMutation.isPending ? 'Updating...' : 'Update Object'}
            </Button>
          </div>
        </form>
      </DialogContent>
